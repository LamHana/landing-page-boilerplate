    ...
    >
      ...
      <article className="col-span-8 max-xl:col-span-1 flex flex-col gap-6">
        <div className="flex flex-col gap-3">
          ...
          <h1 className="text-3xl font-semibold leading-tight text-primary">{post.title}</h1>
          <time dateTime={post.date} className="text-sm text-muted-foreground">
            {formatDate(post.date)}
          </time>
        ...
        {post.thumbnail && (
          <img src={post.thumbnail} alt={post.title} className="w-full rounded-2xl object-cover max-h-[420px]" />
        )}
        {post.content && (
          <div
