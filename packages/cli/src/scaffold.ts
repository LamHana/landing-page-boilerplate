...
async function mergeOptional(optDir: string, targetDir: string) {
...
function injectIntoMarkers(filePath: string, injectContent: string) {
...
function cleanupMarkersInDir(dir: string) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory() && entry.name !== "node_modules") {
      ...
      const cleaned = content
        ...
        .replace(/^\s*\{\/\* __[A-Z_]+__ \*\/\}\s*\n/gm, "");
      if (cleaned !== content) fs.writeFileSync(fullPath, cleaned);
    }
  }
}

function applyTheme(theme: string, targetDir: string) {
...
function replaceTokensInDir(dir: string, tokens: Record<string, string>) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const fullPath = path.join(dir, entry.name);
    if (entry.isDirectory() && entry.name !== "node_modules") {
...
function mergePackageJsonFragments(targetDir: string, optionals: string[]) {
...
function setupHusky(targetDir: string) {
